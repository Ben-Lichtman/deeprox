...
async fn proxy_auth(
	...
	let creds = request.header("proxy-authorization").unwrap();
	if creds != &format!("Basic {}", base64::encode(PROXY_CREDS)) {
...
async fn handle_connection(client_stream: TcpStream) -> Result<(), Error> {
	...
	let request = match PROXY_AUTH {
		true => proxy_auth(client_stream.clone()).await?,
		false => request,
	};
